...
export class RuleLogsService {
  ...
  listByDate(userId: string, date: string) {
    return this.db
      .select()
      .from(ruleLogs)
      .where(and(eq(ruleLogs.userId, userId), eq(ruleLogs.date, date)));
  }

  async set(userId: string, input: SetRuleStateInput) {
    const [row] = await this.db
      .insert(ruleLogs)
      .values({ ruleId: input.ruleId, userId, date: input.date, state: input.state })
